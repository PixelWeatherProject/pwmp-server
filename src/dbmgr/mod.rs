use crate::{
    cli::DatabaseCommand,
    server::{config::Config, db::setup_database},
};
use color_print::cprintln;
use log::{debug, error, info};
use std::{io::stdin, process::exit};

pub fn main(cmd: DatabaseCommand, config: &Config) {
    match cmd {
        DatabaseCommand::Test => match setup_database(config) {
            Ok(_) => info!("Connection successful"),
            Err(why) => error!("Failed to connect: {why}"),
        },
        DatabaseCommand::Init => {
            debug!("Initializing database pool");
            let client = match setup_database(config) {
                Ok(conn) => conn,
                Err(why) => {
                    error!("Failed to connect: {why}");
                    exit(1);
                }
            };

            info!("Executing migrations");
            match client.run_migrations() {
                Ok(()) => info!("Migrations executed successfully"),
                Err(why) => error!("Failed to execute migrations: {why}"),
            }
        }
        DatabaseCommand::Erase {
            content_only,
            keep_devices,
        } => {
            let client = match setup_database(config) {
                Ok(conn) => conn,
                Err(why) => {
                    error!("Failed to connect: {why}");
                    exit(1);
                }
            };

            info!("Connected to the database");
            confirm_erase(&config.short_database_id());

            match client.erase(content_only, keep_devices) {
                Ok(()) => info!("Success!"),
                Err(why) => error!("Failed to erase database: {why}"),
            };
        }
    }
}

fn confirm_erase(db_ident: &str) {
    const KEY: &str = "yes, do it!";

    cprintln!(
        "\n<red><bold><underline>WARNING:</> <yellow>THIS ACTION WILL COMPLETELE ERASE <underline>ALL DATA</underline> AND <italic>(IF SPECIFIED)</italic> <underline>TABLES</underline> FROM THE DATABASE</> <bright-blue><bold>\"{db_ident}\"</> <yellow><bold>!!!</>"
    );
    cprintln!("\n<blue>TYPE <italic>\"{KEY}\"</italic> TO CONFIRM THIS OPERATION!</>");

    let mut buf = String::new();
    stdin().read_line(&mut buf).unwrap_or_default();

    if buf.trim_end() != KEY {
        info!("Operation cancelled, nothing was done.");
        exit(1);
    }
}
