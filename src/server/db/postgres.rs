...
impl super::DatabaseBackend for PostgresClient {
    ...
    #[tracing::instrument(name = "PostgresClient::devices()", level = "debug", skip(self), err)]
    async fn devices(&self) -> Result<Box<[DeviceDescriptor]>, Error> {
        let result: Vec<DeviceDescriptor> =
            sqlx::query_as(include_str!("../../../queries/postgres/get_devices.sql"))
                .fetch_all(&self.0)
                .await?;
    ...
    #[tracing::instrument(
        name = "PostgresClient::node_measurements()",
        level = "debug",
        skip(self),
        err
    )]
    async fn node_measurements(&self, node: NodeId) -> Result<Box<[CompleteMeasurement]>, Error> {
        let result: Vec<CompleteMeasurement> = sqlx::query_as(include_str!(
            "../../../queries/postgres/get_node_measurements.sql"
        ))
        .bind(node)
        .fetch_all(&self.0)
        .await?;
